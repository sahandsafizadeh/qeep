...
func TestXavierUniform(t *testing.T) {
	tensor.RunTestLogicOnDevices(func(dev tensor.Device) {
		...
		t.Run("NewXavierUniform(&XavierUniformConfig{FanIn: 16, FanOut: 4}) / Init([32, 32]) / returns tensor with correct shape and non-nil gradient after backprop", func(t *testing.T) {
		...
		t.Run("NewXavierUniform(nil) / returns error: nil config", func(t *testing.T) {
		...
		t.Run("NewXavierUniform(&XavierUniformConfig{FanIn: 0, FanOut: 1}) / returns error: non-positive FanIn", func(t *testing.T) {
		...
		t.Run("NewXavierUniform(&XavierUniformConfig{FanIn: -1, FanOut: 1}) / returns error: non-positive FanIn", func(t *testing.T) {
			_, err := initializers.NewXavierUniform(&initializers.XavierUniformConfig{
				FanIn:  -1,
				FanOut: 1,
			})
		...
		t.Run("NewXavierUniform(&XavierUniformConfig{FanIn: 1, FanOut: 0}) / returns error: non-positive FanOut", func(t *testing.T) {
			_, err := initializers.NewXavierUniform(&initializers.XavierUniformConfig{
				FanIn:  1,
				FanOut: 0,
			})
		...
		t.Run("NewXavierUniform(&XavierUniformConfig{FanIn: 1, FanOut: -1}) / returns error: non-positive FanOut", func(t *testing.T) {
			_, err := initializers.NewXavierUniform(&initializers.XavierUniformConfig{
				FanIn:  1,
				FanOut: -1,
			})
