...
func validate(epochs int, batches int) (err error) {
	if epochs <= 0 {
		err = fmt.Errorf("expected the number of epochs to be positive: (%d) <= 0", epochs)
		return
	}

	if batches <= 0 {
		err = fmt.Errorf("expected the number of batches to be positive: (%d) <= 0", batches)
		return
	}
