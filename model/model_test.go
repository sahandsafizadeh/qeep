...
func TestModel(t *testing.T) {
	tensor.RunTestLogicOnDevices(func(dev tensor.Device) {
		...
		t.Run("ReLU model with W=-2 B=-1 after Fit for 2 epochs / Predict / returns [0, 0, 0, 0.75]", func(t *testing.T) {
			// ----- given -----
			wInitializer := initializers.NewFull(&initializers.FullConfig{Value: -2.})
			bInitializer := initializers.NewFull(&initializers.FullConfig{Value: -1.})
			...
			// ----- then -----
			exp, err := tensor.Of([][]float64{{0.}, {0.}, {0.}, {0.75}}, &tensor.Config{Device: dev})
			if err != nil {
				t.Fatal(err)
			}
		...
		t.Run("ReLU model with W=-2 B=-1 / Eval after Fit / cumulative MSE goes 0.640625", func(t *testing.T) {
			// ----- given -----
			wInitializer := initializers.NewFull(&initializers.FullConfig{Value: -2.})
			bInitializer := initializers.NewFull(&initializers.FullConfig{Value: -1.})
			...
			// ----- then -----
			if val := result["MSE"]; !(0.64-1e-10 < val && val < 0.65+1e-10) {
				t.Fatalf("expected metric value to be (0.640625): got (%f)", val)
			}
		})
