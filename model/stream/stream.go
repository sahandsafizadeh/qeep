...
func NewStream(compInitFunc node.ComponentInitializerFunc, xs []*Stream) (y *Stream) {
	errCtx := make([]error, 0)
...
func NextStreamFunc1(compInitFunc node.ComponentInitializerFunc) StreamFunc1 {
	return func(x1 *Stream) *Stream {
		return NewStream(compInitFunc, []*Stream{x1})
	}
}

func NextStreamFunc2(compInitFunc node.ComponentInitializerFunc) StreamFunc2 {
	return func(x1, x2 *Stream) *Stream {
		return NewStream(compInitFunc, []*Stream{x1, x2})
	}
}

func NextStreamFunc3(compInitFunc node.ComponentInitializerFunc) StreamFunc3 {
	return func(x1, x2, x3 *Stream) *Stream {
		return NewStream(compInitFunc, []*Stream{x1, x2, x3})
	}
}

func NextStreamFunc4(compInitFunc node.ComponentInitializerFunc) StreamFunc4 {
