...
func TestIntegrationAccessorsShapeModifiers(t *testing.T) {
	tensor.RunTestLogicOnDevices(func(dev tensor.Device) {
		...
		t.Run("Of([2,3,4]) values 0..23 | Slice([1,2),[],[2,4)) then Reshape([3,2]) | compacts non-contiguous slice row-major", func(t *testing.T) {
			...
			y, err := x.Slice([]tensor.Range{{From: 1, To: 2}, {}, {From: 2, To: 4}})
			if err != nil {
				t.Fatal(err)
			}
			y, err = y.Reshape([]int{3, 2})
			if err != nil {
				t.Fatal(err)
			}
		...
		t.Run("Of([2,3,4]) values 0..23 | Slice([],[1,3),[0,2)) then Transpose() | order matters", func(t *testing.T) {
			...
			y, err := x.Slice([]tensor.Range{{}, {From: 1, To: 3}, {From: 0, To: 2}})
			if err != nil {
				t.Fatal(err)
			}
			y, err = y.Transpose()
		...
		t.Run("Of([2,3,4]) values 0..23 | Slice([1,2),[0,2),[1,3)) then UnSqueeze(0) | reshape delegate reaches fallback", func(t *testing.T) {
			...
			y, err := x.Slice([]tensor.Range{{From: 1, To: 2}, {From: 0, To: 2}, {From: 1, To: 3}})
			if err != nil {
				t.Fatal(err)
			}
			y, err = y.UnSqueeze(0)
		...
		t.Run("Of([2,3,4]) values 0..23 | Slice([1,2),[0,2),[2,4)) then Broadcast([3,2,2]) | broadcasts a sliced size-1 leading dim", func(t *testing.T) {
			...
			y, err := x.Slice([]tensor.Range{{From: 1, To: 2}, {From: 0, To: 2}, {From: 2, To: 4}})
			if err != nil {
				t.Fatal(err)
			}
			y, err = y.Broadcast([]int{3, 2, 2})
			if err != nil {
				t.Fatal(err)
			}
		...
		t.Run("Of([2,3,4]) values 0..23 | Transpose() then Slice([],[1,3),[0,2)) | order matters", func(t *testing.T) {
			...
			y, err = y.Slice([]tensor.Range{{}, {From: 1, To: 3}, {From: 0, To: 2}})
			if err != nil {
				t.Fatal(err)
			}
		...
		t.Run("Of([3,4]) values 0..11 | Transpose() then Flatten(0) | flatten reflects transposed order", func(t *testing.T) {
			...
			h, err := tensor.Of([]float64{0., 4., 8., 1., 5., 9., 2., 6., 10., 3., 7., 11.}, &tensor.Config{Device: dev})
			if err != nil {
				t.Fatal(err)
			}
		...
		t.Run("Of([3]) values 1,2,3 | Broadcast([2,4,3]) then Slice([0,1),[1,3),[]) | new leading dim then slice", func(t *testing.T) {
			...
			y, err := x.Broadcast([]int{2, 4, 3})
			if err != nil {
				t.Fatal(err)
			}
			y, err = y.Slice([]tensor.Range{{From: 0, To: 1}, {From: 1, To: 3}, {}})
			if err != nil {
				t.Fatal(err)
			}
		...
		t.Run("Of([1,3]) values 10,20,30 | Broadcast([4,3]) then Transpose() | transposes a broadcasted view", func(t *testing.T) {
			x, err := tensor.Of([][]float64{{10., 20., 30.}}, &tensor.Config{Device: dev})
			if err != nil {
				t.Fatal(err)
			}

			y, err := x.Broadcast([]int{4, 3})
			if err != nil {
				t.Fatal(err)
			}
			y, err = y.Transpose()
		...
		t.Run("Of([1,3]) values 10,20,30 | Broadcast([4,3]) then Reshape([12]) | numElems>len feeds compaction", func(t *testing.T) {
			...
			y, err = y.Reshape([]int{12})
			if err != nil {
				t.Fatal(err)
			}

			h, err := tensor.Of([]float64{10., 20., 30., 10., 20., 30., 10., 20., 30., 10., 20., 30.}, &tensor.Config{Device: dev})
			if err != nil {
				t.Fatal(err)
			}
		...
		t.Run("Of([4,5]) | Slice then Reshape then Transpose then UnSqueeze then Slice | alternating reshape and copy-forcing ops", func(t *testing.T) {
			...
			y, err = y.Slice([]tensor.Range{{}, {}, {From: 0, To: 2}})
			if err != nil {
				t.Fatal(err)
			}
		...
		t.Run("Of([2,3,4]) | Transpose then Slice then Squeeze then Reshape then Transpose | distinct ordering", func(t *testing.T) {
			...
			y, err = y.Slice([]tensor.Range{{}, {From: 0, To: 2}, {From: 0, To: 1}})
			if err != nil {
				t.Fatal(err)
			}
			y, err = y.Squeeze(2)
		...
		t.Run("Of([3,4]) values 0..11 | Transpose then Reshape([1,12]) then Broadcast([3,12]) then Slice([0,2),[2,5)) | broadcasts a reshaped transpose then slices", func(t *testing.T) {
			...
			y, err = y.Reshape([]int{1, 12})
			if err != nil {
				t.Fatal(err)
			}
			y, err = y.Broadcast([]int{3, 12})
			if err != nil {
				t.Fatal(err)
			}
			y, err = y.Slice([]tensor.Range{{From: 0, To: 2}, {From: 2, To: 5}})
			if err != nil {
				t.Fatal(err)
			}
		...
		t.Run("Of([1,3]) values 7,8,9 | Broadcast([4,3]) then Transpose() then Reshape([12]) | zero-stride survives two views", func(t *testing.T) {
			x, err := tensor.Of([][]float64{{7., 8., 9.}}, &tensor.Config{Device: dev})
			if err != nil {
				t.Fatal(err)
			}
			...
			h, err := tensor.Of([]float64{7., 7., 7., 7., 8., 8., 8., 8., 9., 9., 9., 9.}, &tensor.Config{Device: dev})
			if err != nil {
				t.Fatal(err)
			}
		...
		t.Run("Of([2,3,4]) | Broadcast then Slice then Transpose then Flatten | broadcast feeds a long non-contiguous chain", func(t *testing.T) {
			...
			y, err := x.Broadcast([]int{2, 2, 3, 4})
			if err != nil {
				t.Fatal(err)
			}
			y, err = y.Slice([]tensor.Range{{From: 1, To: 2}, {}, {From: 1, To: 3}, {From: 0, To: 2}})
			if err != nil {
				t.Fatal(err)
			}
			y, err = y.Transpose()
			...
			h, err := tensor.Of([]float64{4., 8., 5., 9., 16., 20., 17., 21.}, &tensor.Config{Device: dev})
			if err != nil {
				t.Fatal(err)
			}
