...
func TestChainGrad(t *testing.T) {
	runTestLogicOnDevices(func(dev qti.Device) {
		...
		a, err := qti.Full([]int32{3, 2}, 1., conf)
		if err != nil {
			t.Fatal(err)
		}
		...
		exp, err := qti.Full([]int32{3, 2}, 30., conf)
		if err != nil {
			t.Fatal(err)
		}
...
func TestGradAccumulation(t *testing.T) {
	runTestLogicOnDevices(func(dev qti.Device) {
		...
		a, err := qti.Full([]int32{2, 3}, 1., conf)
		if err != nil {
			t.Fatal(err)
		}
		...
		exp, err := qti.Full([]int32{2, 3}, 10., conf)
		if err != nil {
			t.Fatal(err)
		}
...
func TestUntrackedPaths(t *testing.T) {
	runTestLogicOnDevices(func(dev qti.Device) {
		...
		a, err := qti.Full([]int32{2, 3}, 3., confT)
		if err != nil {
			t.Fatal(err)
		}

		b, err := qti.Full([]int32{2, 3}, 2., confU)
		if err != nil {
			t.Fatal(err)
		}

		c, err := qti.Full([]int32{2, 3}, 1., confU)
		if err != nil {
			t.Fatal(err)
		}
